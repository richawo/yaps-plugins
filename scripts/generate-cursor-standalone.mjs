...
if (catalog.runtimePackage && !/^yaps-cursor-runtime@\d+\.\d+\.\d+$/.test(catalog.runtimePackage)) {
  throw new Error("The registry runtime must use an exact yaps-cursor-runtime version.");
}
const runtimeReference = catalog.runtimePackage || `https://codeload.github.com/richawo/yaps-plugins/tar.gz/${catalog.runtimeCommit}`;
const runtimeDescription = catalog.runtimePackage
  ? `[${catalog.runtimePackage}](https://www.npmjs.com/package/yaps-cursor-runtime/v/${catalog.runtimePackage.split("@")[1]})`
  : `[${catalog.runtimeCommit}](https://github.com/richawo/yaps-plugins/commit/${catalog.runtimeCommit})`;
...
for (const plugin of catalog.plugins) {
  ...
  const config = { command: "npx", args: ["--yes", "--package", runtimeReference, plugin.profile === "memory" ? "yaps-plugin-memory" : "yaps-plugin-media"], env };
