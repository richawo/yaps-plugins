...
When `yaps --help` lists `jobs` and `batch`, run recordings longer than about 10 minutes, and every `maximum` pass, as a background job so a host command timeout cannot interrupt it:

```text
yaps --pretty audio clean "<input>" --quality recommended --output "<output>.wav" --detach --job-label "audio clean"
...
For several files, write one argument array per line to a JSONL manifest, without the `yaps` executable and with absolute paths, for example `{"args":["audio","clean","/abs/interview.m4a","--quality","recommended"],"key":"<file name>"}`. The `key` lets a re-run skip items that already succeeded. Run `yaps --pretty batch <manifest.jsonl> --wait --timeout-secs 90`, then `jobs wait` any items still pending.
