...
- Install Node.js 20+ and, from a git checkout, run `npm ci` in `cursor/yaps` to install the locked MCP SDK dependencies. `bun install --frozen-lockfile` is also supported.
...
The `yaps` MCP entry launches `scripts/launch.mjs`; `yaps-memory` launches `scripts/launch-memory.mjs` and hands the protocol to the installed native vault connector. These are components of one plugin, not separate products or listings. The media helper snapshot stays identical to `mcpb/yaps/server`.
