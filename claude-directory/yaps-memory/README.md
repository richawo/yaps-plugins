# Yaps Memory for Claude Code

Find or save notes in your Yaps memory vault, including past dictations. New users: install Yaps and sign in.
...
- "Find my notes about the product launch and cite the relevant files."
- "Remember this project decision in my Yaps vault."
- "Update the note I selected with these new meeting decisions."
...
Private memory uses the Claude Code identity in Yaps Settings, Agent Access. Read and write grants are separate and never enabled automatically. Stop on a permission denial; do not use filesystem or CLI access to bypass it.
...
If setup fails, open Yaps, check account access and feature readiness, then ask Claude to call vault_status. A missing command requires a Yaps update. Do not work around a permission denial or overwrite an existing output.
