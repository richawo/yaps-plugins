...
// cursor/yaps/scripts/memory-runtime.mjs
import { accessSync as accessSync2, constants as constants2, realpathSync as realpathSync2, statSync as statSync2 } from "node:fs";
...
function connectorPathCandidates({ platform, env }) {
  ...
  return (env.PATH || "").split(win32.delimiter).filter(Boolean).map((directory) => ({ path: win32.join(directory, "yaps_mcp.exe"), source: "path" }));
}
...
function classifyCliResolutionFailure(session) {
...
function resolveYapsConnector(options = {}) {
  const platform = options.platform || process.platform;
  const canAccess = options.canAccess || ((path) => defaultCanAccess(path, platform));
  const candidates = connectorCandidates(options).filter((candidate) => platform !== "win32" || win32.basename(candidate.path).toLowerCase() === "yaps_mcp.exe");
...
// cursor/yaps/scripts/memory-runtime.mjs
async function prepareMemoryLaunch(options = {}) {
  const env = options.env || process.env;
  const platform = options.platform || process.platform;
  ...
  const path = platform === "win32" ? win322 : posix2;
  const canAccess = options.canAccess || ((candidate) => {
    try {
      accessSync2(candidate, platform === "win32" ? constants2.F_OK : constants2.X_OK);
  ...
  if (env.YAPS_MCP_BINARY?.trim() || env.YAPS_INSTALL_DIR?.trim()) {
    ...
    let cliPath = cli.path;
    try {
      cliPath = (options.canonicalize || realpathSync2)(cliPath);
    } catch {
    }
  ...
  return {
    ...
    env: {
      ...
      YAPS_MCP_CLIENT_ID: ["claude-code", "claude_code"].includes(env.YAPS_PLUGIN_HOST) ? "claude-code" : !env.YAPS_PLUGIN_HOST || env.YAPS_PLUGIN_HOST === "cursor" ? "cursor" : "local-mcp",
...
async function startMemory(options = {}) {
  ...
  const child = (options.spawnProcess || spawn2)(launch.command, [], {
    env: launch.env,
...
// cursor/yaps/scripts/launch-memory.mjs
