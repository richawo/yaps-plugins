...
This directory edition includes 11 focused workflows. It does not expose image generation, speech synthesis, the general-purpose Yaps skill, or unrestricted media tools. Its MCP profile rejects calls outside this scope. The full Yaps toolkit remains available in the separate `yaps-all` package.

Private memory uses a separate local MCP connector and the Claude Code identity in Yaps Settings, Agent Access. It never auto-authorizes reads or writes. The user controls those grants independently. Only read requested notes, and make changes when the user asks. Stop on a permission denial; never use filesystem or CLI access to bypass it.
